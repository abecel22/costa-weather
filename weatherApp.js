...
function init() {
    ...
    // uses the coordinates on the weatherUnderground API URL
	  ...
	  var urlFromIP= "https://api.wunderground.com/api/d26f80d16a35b851/geolookup/conditions/q/"+ coordinates+ ".json";
      $(document).ready(function(){
		...
		$.ajax({
			...
			success: function(parsed_json) {
				...
				$("#dispWeatherText").text("Current Condition: " + weatherText);
				changeImage();
			} 
		});
		}
...
//Change background image if not weather is not clear and sunny.
		function changeImage(){
			if (weatherText.includes("Cloudy")){
				$("body").css("background-image", "url('https://www.dropbox.com/s/klqz32uwj182swc/partlyCloudy.jpg?raw=1')");
			} else if(weatherText.includes("Rain")){
				$("body").css("background-image", "url('https://www.dropbox.com/s/54gbl3x4v0s7g0c/rain.jpg?raw=1')");
			} else if(weatherText === "Overcast"){
				$("body").css("background-image", "url('https://www.dropbox.com/s/1hutuhwm8hyoeon/overcast.jpg?raw=1')");
			}else if(weatherText.includes("Thunderstorm")){
				$("body").css("background-image", "url('https://www.dropbox.com/s/2j97szsjl8rznlt/thunder.jpg?raw=1')");
			}else if (weatherText === "Clear"){
				$("body").css("background-image", "url('https://www.dropbox.com/s/4tjppcuqzsculsv/clearSky.jpg?raw=1')");
			}
		}
  ...
  $.getJSON("https://autocomplete.wunderground.com/aq?query=" + value + "&cb=?", function(data) {
    ...
    $.each(data.RESULTS, function(key, val) {
      if (val.name.search(rExp) != -1 && key < 11) {
        output += '<p>';
        output += '<a href="#newCity" data-q_id='+ val.zmw+ '>'+ val.name +'</a>';
        output += '</p>';
      }
      output+= '</div>';
    }); // end each
    $("#searchResults").html(output);
     ...
     $("a").on("click", function (e) {
     	...
     	urlFromIP= "https://api.wunderground.com/api/d26f80d16a35b851/conditions/q/zmw:"+newCityCode +".json";
     	function getNewWeather() {
		$.ajax({
			url: urlFromIP,
			dataType: "jsonp",
			success: function(parsed_json) {
				locationName = parsed_json.current_observation.display_location.city;
				tempF= parsed_json.current_observation.temp_f;
				tempC= parsed_json.current_observation.temp_c;
				humidity = parsed_json.current_observation.relative_humidity;
				weatherText = parsed_json.current_observation.weather;
